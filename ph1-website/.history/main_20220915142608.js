...
function create(questionNumber,questionText,selectArray){
    ...
    <h3 class="yes">正解！</h3>
    <span class="A">A</span>
    <span>${shuffledQuestion[questionNumber-1].select[question[questionNumber-1].answer]}</span>
    </div>

    <div class="x-answer-box" id="x-answer-box-${questionNumber}">
    <h3 class="no">不正解...</h3>
    <span class="A">A</span>
    <span class"">${shuffledQuestion[questionNumber-1].select[question[questionNumber-1].answer]}</span>
    </div>
    ${noteHtml}
    </div>
    </section>`
