...
function create(questionNumber,questionText,selectArray){
    ...
    const noteHtml=question[questionNumber-1].note ? `
        <cite class="cite">
        <div>
        <img src="./assets-ph1-website-main 2/img/icon/icon-note.svg">
        </div>
        ${question[questionNumber-1].note}
        </cite>
    `: "";
    let quiz = `
    ...
    <h3 class="no">不正解...</h3>
    <span class="A">A</span>
    <span class"">${question[questionNumber-1].select[question[questionNumber-1].answer]}</span>
    </div>
    ${noteHtml}
    </div>
    </section>`
